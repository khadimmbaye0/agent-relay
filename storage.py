...
Every operation runs inside one PostgreSQL transaction, and operations that
contend for a row (claim, recovery, heartbeat, terminal submission) select that
row ``FOR UPDATE`` so a race has exactly one consistent outcome.
"""
...
def authenticate(token: str) -> Agent:
    ...
    # last_seen_at is an authenticated observation and therefore a write.  No
    # extra coordination is needed: PostgreSQL blocks only the competing update
    # of this one agent row while the surrounding transaction runs.
    with db_session() as db:
...
def _reuse_task(existing: Task, recipient_id: str, input_text: str) -> dict[str, str]:
...
def _task_for_idempotency_key(sender_id: str, idempotency_key: str | None) -> Task | None:
...
def _insert_task(
    sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None
) -> dict[str, str]:
    with db_session() as db:
        if db.get(Agent, recipient_id) is None:
        ...
        if idempotency_key is not None:
            ...
            if existing is not None:
                return _reuse_task(existing, recipient_id, input_text)
        task = Task(
            id=new_id("task"),
            sender_id=sender_id,
...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    try:
        return _insert_task(sender_id, recipient_id, input_text, idempotency_key)
    except IntegrityError:
        # Two API processes can pass the lookup above at the same moment; the
        # unique constraint on (sender_id, idempotency_key) then rejects one of
        # them.  That request must report the task the winner created, which is
        # what keeps a retried submission from duplicating work.
        existing = _task_for_idempotency_key(sender_id, idempotency_key)
        if existing is None:
            raise
        return _reuse_task(existing, recipient_id, input_text)
...
def claim_one(agent_id: str, worker_id: str | None) -> dict[str, Any] | None:
    with db_session() as db:
        ...
        # The oldest queued task, locked for the duration of this transaction.
        # SKIP LOCKED lets a concurrent claimer take the next row instead of
        # waiting for this one, so every task has exactly one active lease
        # without a process-local queue or a serialized writer.
        task = db.scalar(
            select(Task)
...
def _find_attempt_for_token(db: Session, task_id: str, token: str) -> Attempt | None:
    # Locked: heartbeat and terminal submission both read the attempt and then
    # decide based on it.  Holding the row until commit means two concurrent
    # requests cannot both act on the same live claim.
    return db.scalar(
        select(Attempt)
    ...
    with db_session() as db:
        ...
        if attempt.outcome in {"completed", "failed"}:
            if attempt.terminal_action == action and attempt.terminal_payload_hash == value_digest:
                # Acquiring the attempt lock may have waited for another
                # process to commit, so report the persisted task state rather
                # than the snapshot read before the lock.
                db.refresh(task)
                return {"task_id": task.id, "status": task.status}
